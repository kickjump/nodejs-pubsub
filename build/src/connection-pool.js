...
class ConnectionPool extends events_1.EventEmitter {
    ...
    close(callback) {
        ...
        this.failedConnectionAttempts = 0;
        this.noConnectionsTime = 0;
        each(connections, (connection, onEndCallback) => {
            connection.end(err => {
                connection.cancel();
                onEndCallback(err);
            });
    ...
    createConnection() {
        this.getClient((err, client) => {
            ...
            requestStream.on('status', status => setImmediate(onConnectionStatus, status));
            connection.on('error', onConnectionError)
                .on('data', onConnectionData)
                .write({
                subscription: projectify_1.replaceProjectIdToken(this.subscription.name, this.pubsub.projectId),
